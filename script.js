...
async function mediaAssociationCards(artist, entity, kind, listeningStat) {
  ...
  return (payload.associations || []).filter((association) => {
    ...
    return `<article class="media-source" data-media-source="${escapeHtml(source.label)}" data-card-id="media-${encodeURIComponent(`${kind}-${artist}-${entity}-${association.source}-${association.title}-${association.excerpt}`)}"><a class="media-main" href="${association.url}" target="_blank" rel="noreferrer" aria-label="Open ${escapeHtml(association.source)} media association"></a><small class="personal-stat">${escapeHtml(listeningStat)}</small><span class="media-type">${escapeHtml(association.kind)}</span><span class="media-brand">${logo}<b>${escapeHtml(source.label)}</b></span><p>${escapeHtml(association.title)}</p><div class="media-coverage">${coverage}</div><span class="media-origin">${escapeHtml(entity)} · ${escapeHtml(artist)}</span><a class="media-open" href="${association.url}" target="_blank" rel="noreferrer" aria-label="Open ${escapeHtml(association.source)} media association">↗</a></article>`;
  });
}
